...
- **ATPV-e MG** (`/api/v1/atpve-mg/*`): proxy transparente 1:1 para a API ATPV-e MG da Chekaki via `proxyAtpveMgExternal` (substituiu os antigos `POST /api/v1/detran-mg/intencao-venda` e `POST /api/v1/detran-mg/atpve` da Infosimples). 9 rotas espelhando a upstream: `GET /api/v1/atpve-mg` (listar), `POST .../cadastrar`, `GET .../:id`, `GET .../:id/pdf`, `GET .../:id/pdf/base64`, `POST .../:id/atualizar`, `POST .../:id/registrar`, `POST .../:id/excluir`, `GET .../protocolo/:protocolo`. Só o `cadastrar` cobra (`EXTERNAL_API_PRICE`), após sucesso da upstream; as demais rotas gerenciam pedido já pago e são gratuitas. Respostas (PDF ou JSON) são repassadas como vêm da upstream.
