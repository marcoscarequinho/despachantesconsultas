...
- **O número do cartão não passa por aqui**: os campos são iframes do Card Payment Brick do Mercado Pago ([assets/pagamento-cartao.js](assets/pagamento-cartao.js)) e o servidor recebe só o token de uso único — é o que mantém o site fora do escopo PCI. **Não existe "escanear cartão" pela câmera**: os campos em `secure-fields.mercadopago.com` vêm com `autocomplete="off"` (verificado no HTML deles), então nem o scanner nem o autofill do celular aparecem, e não dá para mudar de fora do iframe. A única forma de ter isso seria trocar o brick pelo `mp.cardForm()` com inputs nossos — o que traria o número do cartão para dentro do nosso JavaScript e levaria o site de SAQ A para SAQ A-EP no PCI. É uma decisão de negócio, não um ajuste de código.
- **Um brick por tipo**: o total já aparece no botão do formulário, então cada aba monta o seu (`customization.paymentMethods.types.excluded` tira o outro tipo).
