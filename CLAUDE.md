...
- **Consulta avulsa** (`/consulta-avulsa` + [consulta-avulsa.html](consulta-avulsa.html)): fluxo público sem cadastro — `POST /api/public/pedido` valida os campos e cria PIX no Mercado Pago (tabela `public_orders`); `GET /api/public/pedido/:token` faz polling, confirma o pagamento e só então executa a consulta na Infosimples (claim atômico `UPDATE ... WHERE status='PENDING'` impede execução dupla). Serviços em `PUBLIC_PAY_SERVICES`, mesmo preço de R$ 5,00. As páginas exigem **código de acesso por cliente** (tabela `public_access_codes` com coluna `page`, gerado no admin em Consultas Avulsas; link `?codigo=XXXXXX` validado no servidor antes de servir o HTML — sem código volta para a home). Duas páginas privadas: `/consulta-avulsa` (`page='avulsa'`, QR na tela) e `/consulta-whatsapp` (`page='whatsapp'`, [consulta-whatsapp.html](consulta-whatsapp.html) — recebe o WhatsApp do pagador e envia QR + copia-e-cola via Z-API `sendWhatsAppImage`/`sendWhatsApp`; ao concluir, avisa no WhatsApp com o link do documento). O código usado e o telefone ficam gravados em cada pedido (`public_orders.access_code`/`phone`). Páginas com `noindex` (meta + X-Robots-Tag + robots.txt) — divulgadas só por link direto.
